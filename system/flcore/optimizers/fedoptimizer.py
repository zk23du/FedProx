        ...
        for group in self.param_groups:
            ...
            for param in group['params']:
                ...
                for p, g in zip(group['params'], global_params):
                    ...
                    d_p = p.grad.data + group['mu'] * (p.data - g.data)  ################## prox part
                            # Use global_params
...
#                 delta_t, r_t = w_dict[param]['delta_t'], w_dict[param]['r_t']
#                 for p, g in zip(group['params'], global_params):
#                     regularization_term = group['mu'] * (p.data - g.data)
...
#         self.gamma = float((num / (denom + self.eps)).clamp(min=0, max=1))
    ...
    #                 delta_t, r_t = w_dict[param]['delta_t'], w_dict[param]['r_t']
    #                 regularization_term = mu * torch.norm(p.data - g.data)**2
    #                 num -= lr * torch.sum(delta_t * r_t) + regularization_term
    #                 denom += lr * delta_t.norm() ** 2 + regularization_term
